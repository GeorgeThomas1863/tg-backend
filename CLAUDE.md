...
- Run dev server: `uv run uvicorn main:app --reload --env-file ../.env` (`.env` lives at repo root; `--env-file` works because `uvicorn[standard]` pulls in `python-dotenv` as a transitive dependency — `config.py` itself just reads `os.environ` directly and does no dotenv loading of its own)
...
- `main.py` — HTTP only: routes, Range-header parsing, status codes. Delegates everything Telegram-related to `telegram.py`.
